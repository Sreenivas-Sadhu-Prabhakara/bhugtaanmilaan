...
func TestPostgresStore_RoundTrip(t *testing.T) {
	...
	in, _ := json.Marshal(Payment{Channel: "upi", Amount: 250})
	saved, err := store.Save(Record{Input: in, Headline: 250, Label: "upi"})
	if err != nil {
		t.Fatalf("save: %v", err)
	}
	if saved.ID == 0 {
	...
	if len(items) == 0 || items[0].ID != saved.ID || items[0].Label != "upi" {
